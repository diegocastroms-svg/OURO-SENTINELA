...
COOLDOWN_MIN = int(os.getenv("COOLDOWN_MIN", "60"))       # +tempo entre alertas
MIN_QV_USDT = float(os.getenv("MIN_QV_USDT", "7000000"))  # +liquidez mínima
MIN_FORCE = int(os.getenv("MIN_FORCE", "80"))              # +força mínima
BOOK_MIN_BUY = float(os.getenv("BOOK_MIN_BUY", "1.45"))    # +exigente
BOOK_MIN_SELL = float(os.getenv("BOOK_MIN_SELL", "0.70"))
...
@app.route("/")
def home():
    return "OURO-TENDÊNCIA v1.1 – Online", 200
...
# UTILITÁRIOS
# =========================
def br_time(): return datetime.now().strftime("%H:%M:%S")
...
def macd(values, fast=12, slow=26, signal=9):
    if len(values)<slow+signal: return 0,0
...
def book_ratio(depth):
    def power(levels): return sum(float(p)*float(q) for p,q in levels[:20])
    b,a=power(depth.get("bids",[])),power(depth.get("asks",[]))
...
def calc_force(ema9,ema20,macd_line,hist,rsi_val,ratio):
    ...
    if 45<rsi_val<65: score+=10
    if ratio>=BOOK_MIN_BUY: score+=50
    if ema9<ema20 and macd_line<0 and ratio<=BOOK_MIN_SELL: score=max(score,80)
...
def build_alert(sym,direction,force,ema9,ema20,macd_line,hist,rsi_val,ratio,vol_mult):
    hora=br_time(); seta="🔺" if direction=="alta" else "🔻" if direction=="baixa" else "⚪"
    ...
    insight=("fluxo comprador muito forte — estrutura sólida"
             if direction=="alta" else
             "fluxo vendedor dominante — pressão de baixa"
             if direction=="baixa" else
             "monitorar aproximação de cruzamento")
    return (f"🟢🟢🟢🟢\n"
...
async def scan_once():
    async with aiohttp.ClientSession() as s:
        ...
        for sym in symbols:
            try:
                kl,dp=await asyncio.gather(fetch_klines(s,sym,"5m",120),fetch_depth(s,sym,40))
                closes=[float(k[4]) for k in kl]; vols=[float(k[5]) for k in kl]
                ema9,ema20=ema(closes[-40:],9),ema(closes[-40:],20)
                macd_line,hist=macd(closes); rsi_val=rsi(closes)
                base=statistics.median(vols[-11:-1]) or 1
                vol_mult=vols[-1]/base; ratio=book_ratio(dp)
        ...
        now=datetime.utcnow(); msgs=[]
        for r in results:
            sym=r[0]; nxt=cooldown.get(sym,datetime.min)
            if now<nxt: continue
            msgs.append(build_alert(*r))
            cooldown[sym]=now+timedelta(minutes=COOLDOWN_MIN)
        for m in msgs[:3]:
            print(m.replace("\n"," | ")); await send_telegram(m); await asyncio.sleep(0.3)

async def monitor_loop():
    print("🚀 OURO-TENDÊNCIA v1.1 ativo.")
    while True:
        try: await scan_once(); await asyncio.sleep(SCAN_INTERVAL)
        except Exception as e:
            print("⚠️ Erro:",e); await asyncio.sleep(5)
